...
default_parameters = {
    ...
    'download_name': 'amazing_pattern.svg'
}

current_parameters = default_parameters.copy()

def init_program(root):
...
def run_drawing():
    ...
    corner_change = int(sliders['corner_change'].get())
    mode = mode_var.get()
    colors = current_parameters["colors"]
...
def reset_defaults():
    for key, val in default_parameters.items():
        if key in sliders:
            sliders[key].set(val)
    mode_var.set(default_parameters["mode"])
    name_entry.delete(0, tk.END)
    name_entry.insert(0, default_parameters["download_name"])
...
for label_text, min_val, max_val, step in slider_config:
    ...
    tk.Label(frame, text=label_text.replace("_", " ").title(), width=18, anchor="w").pack(side=tk.LEFT)
    slider = tk.Scale(frame, from_=min_val, to=max_val, orient=tk.HORIZONTAL)
    slider.set(default_parameters[label_text])
    slider.pack(side=tk.LEFT, fill=tk.X, expand=True)
    value_label = tk.Label(frame, text=str(int(slider.get())), width=5, anchor="e")
...
tk.Label(mode_frame, text="Mode:", width=18, anchor="w").pack(side=tk.LEFT)
mode_var = tk.StringVar(value=default_parameters["mode"])
for m in ["normal", "overlapped", "inverse"]:
    tk.Radiobutton(mode_frame, text=m.title(), variable=mode_var, value=m).pack(side=tk.LEFT, padx=5)

# File name
name_frame = tk.Frame(control_frame)
name_frame.pack(fill=tk.X, pady=6)
tk.Label(name_frame, text="SVG file name:", width=18, anchor="w").pack(side=tk.LEFT)
...
tk.Button(btn_frame, text="Run", command=run_drawing).pack(fill=tk.X, pady=3)
tk.Button(btn_frame, text="Download SVG", command=download_svg).pack(fill=tk.X, pady=3)
tk.Button(btn_frame, text="Reset to Defaults", command=reset_defaults).pack(fill=tk.X, pady=3)
